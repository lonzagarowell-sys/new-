  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <form onSubmit={submit} className="bg-white p-8 rounded-2xl shadow w-full max-w-md">
        ...
        <button type="button" onClick={() => signInWithGoogle()} className="w-full bg-red-500 text-white py-2 rounded mb-2">Sign in with Google</button>
        <button type="button" onClick={() => resetPassword(email)} className="text-sm text-blue-600 hover:underline">Forgot password?</button>
      </form>
    </div>
  );
}
